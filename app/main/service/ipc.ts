...
export const handleOpenWidget = () => {
  ipcMain.on(IpcEvent.OPEN, (event: any, args: {name: string, data: any}) => {
...
export const handleFetch = () => {
  ipcMain.handle(IpcEvent.FETCH, async (event: any, args: { method: 'GET' | 'POST', url: string, data: any }) => {
    return await fetch.handle(args.method, args.url, args.data)
  })
}

export const handleUserLogin = () => {
  ipcMain.handle(IpcEvent.USER_LOGIN, async (event: any, args: any) => {
    try {
...
export const handleUpdateUser = () => {
  ipcMain.handle(IpcEvent.UPDATE_USER, async (event: any, args: {email: string, nickname: string, password: string, avatar: string}) => {
    try {
      await saveUser(args)
    } catch (e) {  }
  })
...
export const handleCompile = () => {
  ...
  ipcMain.handle(IpcEvent.COMPILE, (event: any, args: {type: string, code: string}) => {
    return (compiler as any)[args.type].compile(args.code)
  })
}

export const handleOpenFile = () => {
  ipcMain.handle(IpcEvent.OPEN_FILE, async (event: any, args: { type: string }) => {
    if (args.type === 'image') {
...
export const handleSaveFile = () => {
  ipcMain.handle(IpcEvent.SAVE_FILE, async (event: any, args: { type: string, data: any }) => {
    if (args.type === 'base64') {
...
export const handleUserLogout = () => {
  ipcMain.handle(IpcEvent.USER_LOGOUT, async (event: any, args: any) => {
    if (Main.isCreate()) {
      Main.close();
      await clearUser();
      Login.create();
...
export const handleSaveWebGLPageWithSocket = () => {
  ipcMain.handle(IpcEvent.AUTO_SAVE_WEBGL_PAGE, (event: any, args: any) => {
