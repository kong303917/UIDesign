...
export default function ComponentsPanel(props: {}) {
  ...
  const foldElem = ComponentFoldConfig.map((v: any, i: number) => {
    const type = v.type
    if (v.children.length === 0)
      return null

    const items = v.children.map((v: any, i: number) => {
      return (
  ...
  const openDeletePageModal = () => {
    modal(cancel => <Confirm title={`do you want to delete ${webglPage.name}?`} cancel={cancel} confirm={() => {
      deleteOnePage(user.email, webglPage.id).then((v: any) => {
        if (!v.err) {
          fetchOnePage(user.email, v.pageId).then((res: any) => {
            cancel()
...
function WebGLPageList(props: any) {
  ...
  const handleClick = () => {
    if (hideContent) {
      ...
      setPages([])
      setLoading(true)
      fetchAllPages(user.email).then((v: any) => {
        if (!v.err) {
    ...
    setHideContent((hideContent: any) => !hideContent)
  }
