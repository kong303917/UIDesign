...
export function EditorMenu() {
  ...
  const handleDeleteDslFile = () => {
    modal(cancel => <Confirm title={`Do you want to delete ${dslFile.filename}`} cancel={cancel} confirm={() => {
      deleteDslFile(user.email, dslFile.id, dslFile.fileId).then((v: any) => {
        if (!v.err) {
          toast('delete file!')
  ...
  const handleSaveDslCode = () => {
    ...
    saveDslCode(cur.id, user.email, cur.code, cur.fileId).then((v: any) => {
      if (!v.err) {
        toast('save code!');
      }
    })
  ...
  const handleRun = () => {
    Bridge.compile('json', openFileItems.items[openFileItems.currentIndex].code).then((v: any) => {
