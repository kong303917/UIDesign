...
export function dslFileArrayReducer(state: IDSLFileArrayState, action: TActions) {
  switch (action.type) {
    ...
    case DSLFileArrayActions.LOCAL_SAVE_FILE: {
      for (let i: number = 0; i < state.files.length; i++) {
    ...
    case DSLFileArrayActions.REMOVE_FILE: {
      return {
        files: state.files.filter((v: any) => v.id !== action.id)
      }
    }
    default: {
      return {...state}
...
export function openFileItemsReducer(state: IOpenFileItemsState, action: TActions) {
  switch (action.type) {
    case OpenFileItemsActions.ADD_FILE: {
      const has = state.items.some((v: any) => v.id === action.item.id)
      let index
      if (has) {
    ...
    case OpenFileItemsActions.CLOSE_FILE: {
      return {
        ...state,
        currentIndex: 0,
        items: state.items.filter((v: any, i: number) => i !== action.index)
      }
    }
